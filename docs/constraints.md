...
- `C-DAEMON-PROFILE-STORE-EXPERIMENT`: Decision 0011 permits an isolated macOS user profile store with non-secret UUID/name metadata in user Application Support and full synthetic configuration text in the ordinary login Keychain. It has no NetworkExtension ACL and is not connected to the daemon-control protocol, GUI, routes, DNS, PF, installer, automatic startup, or real profiles/runtime.
