...
rebinding. The route-plan model can now describe neutral `excluded` routes so
the future full-tunnel policy has an explicit representation for LAN and
corporate exclusions. An exclusion is not a physical-route instruction: an
active more-specific tunnel may own it. The daemon rejects every such plan,
and every IPv4 default route plan, before its backend starts: UAPI does not
contain `ExcludeIPs`, so the daemon cannot bind a caller-supplied exclusion to
trusted configuration yet. The backend selects one physical IPv4 RIB base route at start and
