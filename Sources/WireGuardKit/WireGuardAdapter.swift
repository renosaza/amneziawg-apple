...
public class WireGuardAdapter {
    ...
    #if os(macOS)
    /// Pending endpoint reset after a network change.
    private var endpointResetWorkItem: DispatchWorkItem?

    /// Coalesces the path-update burst generated by one network change.
    private static let endpointResetDelay: DispatchTimeInterval = .seconds(2)
    #endif
    ...
    public func stop(completionHandler: @escaping (WireGuardAdapterError?) -> Void) {
        workQueue.async {
            ...
            #if os(macOS)
            self.endpointResetWorkItem?.cancel()
            self.endpointResetWorkItem = nil
            #endif
    ...
    #if os(macOS)
    /// Resets peer endpoints after a network change and coalesces path updates.
    private func scheduleEndpointReset() {
        endpointResetWorkItem?.cancel()

        let workItem = DispatchWorkItem { [weak self] in
            guard let self, case .started(let handle, let settingsGenerator) = self.state else { return }

            let previousEndpoints = settingsGenerator.resolvedEndpoints
            let (wgConfig, resolutionResults, endpointsChanged) = settingsGenerator.endpointUapiConfigurationAfterNetworkChange()
            self.logEndpointResolutionResults(resolutionResults)
            guard !wgConfig.isEmpty else { return }

            if endpointsChanged {
                do {
                    // The backend must not use a new endpoint until its /32 or /128
                    // excluded route is installed by this provider's native settings.
                    let networkSettings = settingsGenerator.generateNetworkSettings()
                    self.logNetworkSettingsSummary(networkSettings, context: "network change")
                    try self.setNetworkSettings(networkSettings)
                } catch {
                    settingsGenerator.restoreResolvedEndpoints(previousEndpoints)
                    self.logHandler(.error, "Failed to install routes for re-resolved endpoints: \(error.localizedDescription)")
                    do {
                        try self.setNetworkSettings(settingsGenerator.generateNetworkSettings())
                    } catch {
                        self.logHandler(.error, "Failed to restore routes after endpoint resolution failure: \(error.localizedDescription)")
                    }
                    return
                }
            }

            self.logHandler(.verbose, "Resetting peer endpoints after network change")
            wgSetConfig(handle, wgConfig)
            wgBumpSockets(handle)
        }

        endpointResetWorkItem = workItem
        workQueue.asyncAfter(deadline: .now() + Self.endpointResetDelay, execute: workItem)
    }
    #endif
